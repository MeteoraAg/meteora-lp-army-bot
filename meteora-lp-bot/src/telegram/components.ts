...
import { calculateTokenPercentages, formatNumber } from "../utils/solana";
...
• <b>Pool liquidity:</b> ${formatNumber(parseFloat(pairInfo.liquidity))}
• <b>Daily Yield:</b> ${pairInfo.apr.toFixed(2)}%
• <b>24h volume:</b> ${formatNumber(pairInfo.tradeVolume24h)}
• <b>24h fees:</b> ${formatNumber(pairInfo.fees24h)}
...
export function pairListMessage(pairResponse: PairResponse) {
  ...
  for (var group of pairResponse.groups) {
    ...
    const fee =
      group.pairs.length > 0 ? group.pairs[0].baseFeePercentage : "N/A";
...
• <b>TVL:</b> ${formatNumber(totalTvl)}
• <b>24H Volume:</b> ${formatNumber(totalVolume24h)}
...
export function addLiquidityMessage(
  pairInfo: PairInfo,
  tokenX: string,
  tokenY: string,
